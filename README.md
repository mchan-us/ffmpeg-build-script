...
### CentOS and macOS are no longer supported by this repository. Only Ubuntu is supported.

### Only report build errors that occur on Ubuntu. We do not provide any support for other distributions and macOS.

### If you are looking for an original repository that also supports CentOS and macOS, go here. https://github.com/markus-perl/ffmpeg-build-script

build-ffmpeg
==========

The FFmpeg build script provides an easy way to build a **static** FFmpeg on **Ubuntu** with **non-free and GPL codecs** included.
...
other than Ubuntu 20.04, because I don't have the resources or time to maintain different systems.
...
$ bash <(curl -s "https://raw.githubusercontent.com/mchan-us/ffmpeg-build-script/master/web-install.sh?v1")

# Without GPL and non-free codes, see https://ffmpeg.org/legal.html 
$ bash <(curl -s "https://raw.githubusercontent.com/mchan-us/ffmpeg-build-script/master/web-install-without-gpl-and-non-free.sh?v1")
```
...
```bash
$ git clone https://github.com/mchan-us/ffmpeg-build-script.git
...
* `libdav1d`, High Speed AV1 Decoder (Needs Python 3)
...
* Ubuntu => Focal Fossa, other Distributions might work too
...
These builds cannot be built with the --disable-gpl-and-non-free switch, as CUDA is non-free. See https://ffmpeg.org/legal.html
```bash
export DOCKER_BUILDKIT=1

## Start the build
$ docker build --tag=ffmpeg:cuda-ubuntu -f cuda-ubuntu.dockerfile --build-arg VER=20.04 .
```
...
```bash
$ docker build --output type=local,dest=build -f export.dockerfile --build-arg DIST=ubuntu .
$ ls build
bin lib
$ ls build/bin
ffmpeg ffprobe
...
$ sudo docker run --gpus all ffmpeg-cuda -hwaccel cuda -hwaccel_ouput_format cuda -c:v h264_cuvid -i https://files.coconut.co.s3.amazonaws.com/test.mp4 -c:v hevc_nvenc -vf scale_npp=-1:1080 - > test.mp4
```
...
If you want to enable VDPAU, please refer to [these](#VDPAU-installation) and install the driver.
...
To install the CUDA toolkit on Ubuntu, run below.
...
You will need the libvdpau driver, so please install it below.

```bash
$ sudo apt install libvdpau-dev vdpauinfo
```
...
Options:
  ...
  -b, --build                     Starts the build process
      --disable-gpl-and-non-free  Disable non-free codecs  - https://ffmpeg.org/legal.html
...
* Github: [http://www.github.com/mchan-us/](https://github.com/mchan-us/ffmpeg-build-script)
